...
export class ActiveAuras {
  ...
  static UpdateToken(map, canvasToken, tokenId) {
    ...
    for (const auraEffect of checkEffects) {
      ...
      if (wallsBlock !== "system") {
        ...
        wallsBlock = getAASetting("wall-block");
      }
        ...
        case "token":
          {
            if (auraEffect.parentActorLink) {
              const parentActor = game.actors.get(auraEffect.parentActorId);
              const auraTokenArray = parentActor?.getActiveTokens() ?? [];
            ...
            // Defensive: a stale map entry (e.g. left over from a partial
            // wildshape) can resolve to no auraEntity. Without this guard the
            // next line throws, breaking out of the entire MainAura loop and
            // leaving the canvas frozen at whatever effects were applied last.
            if (!auraEntity) {
              Logger.debug("AA: skipping aura entry with unresolvable auraEntity", { auraEffect });
              continue;
            }
      ...
      if (distance && !auraEffect.data.flags?.ActiveAuras?.Paused) {
        ...
        && Array.from(canvasToken.document.actor?.allApplicableEffects() ?? []).some(
          (e) => AAHelpers.originsMatch(e.origin, auraEffect.data.origin) && e.name === effectName
        )
      ) {
        if (MapObject) {
          MapObject.add = false;
  ...
  static async CreateActiveEffect(tokenID, effectData) {
    ...
    if (duplicateEffect) {
      ...
      // })
      if (AAHelpers.originsMatch(duplicateEffect.origin, effectData.origin)) return;
    ...
    // Stamp the scene this effect was applied in. The tag is used by
    // RemoveCrossSceneSourceAuras to identify cross-scene-tagged effects
    // during eager cleanup paths (source token movement, scene activation).
    // Per-scene RemoveAppliedAuras intentionally does NOT use the tag as a
    // preservation filter -- the activeGM's currently-viewed scene is
    // authoritative for what auras are active on tokens visible there.
  ...
  static async RemoveActiveEffects(tokenID, effectOrigin) {
    ...
    for (const tokenEffect of tokenEffects) {
      if (AAHelpers.originsMatch(tokenEffect.origin, effectOrigin) && tokenEffect.flags?.ActiveAuras?.applied === true) {
        try {
          ...
          Logger.debug(
            aaLocalize("ACTIVEAURAS.RemoveLog", { effectDataName: effectOrigin, tokenName: token.name })
          );
        }
      }
    }
  ...
  static async movementUpdate(token) {
    ...
    // Route through the GM Semaphore (same queue as CollateAuras) so two
    // overlapping updateToken hooks cannot run MainAura concurrently.
    // V13's multi-waypoint movement emits one updateToken per step; when a
    // remote player moves, those hooks can arrive close enough that hook #1's
    // MainAura is still awaiting createEmbeddedDocuments while hook #2's
    // MainAura runs its CreateActiveEffect dedup check. Neither lookup sees
