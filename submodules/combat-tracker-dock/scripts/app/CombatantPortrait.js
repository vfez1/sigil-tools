...
export class CombatantPortrait {
    ...
    async renderInner() {
        ...
        const template = await foundry.applications.handlebars.renderTemplate(`${TEMPLATE_PATH}/combatant-portrait.hbs`, { ...data });
        ...
        if (combatant.actor) {
            for (const effect of combatant.actor.temporaryEffects) {
                if (effect.statuses.has(CONFIG.specialStatusEffects.DEFEATED)) turn.defeated = true;
                    ...
                    const percent = effect.duration?.remaining / (effect.duration?.seconds ?? effect.duration?.duration);
