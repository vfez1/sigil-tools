...
async function _callMacro(effect, type = "never", context = {}) {
  const script = foundry.utils.getProperty(effect, `flags.${effectmacro.id}.${type}.script`);
  if (!script) {
...
export function hasMacro(effect, type = "never") {
  return !!foundry.utils.getProperty(effect, `flags.${effectmacro.id}.${type}.script`);
}
...
export async function removeMacro(effect, type = "never") {
  const script = foundry.utils.getProperty(effect, `flags.${effectmacro.id}.${type}`);
  if (!script) return null;
  return effect.update({ [`flags.${effectmacro.id}.-=${type}`]: null });
}
...
export async function createMacro(effect, type = "never", script) {
  ...
  if (script instanceof Function) {
    return effect.update({ [`flags.${effectmacro.id}.${type}.script`]: `(\n        ${script.toString()}\n      )()` });
  } else {
    return effect.update({ [`flags.${effectmacro.id}.${type}.script`]: script.toString() });
  }
}
...
export async function updateMacro(effect, type = "never", script) {
  ...
  } else if (script.toString() !== foundry.utils.getProperty(effect, `flags.${effectmacro.id}.${type}.script`)) {
